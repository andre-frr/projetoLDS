...
Este projeto é uma aplicação web de gestão académica, desenhada para gerir informações sobre docentes, cursos, unidades curriculares (UCs) e outras entidades académicas. A arquitetura é baseada em microserviços, com comunicação via GraphQL e gRPC.
...
-   `graphql/`: Um serviço de API GraphQL que atua como um gateway, agregando dados de outros serviços e expondo-os a um único endpoint.
...
-   `db/`: Contém o script de inicialização (`init.sql`) para a base de dados PostgreSQL, definindo o esquema e inserindo dados iniciais.
-   `docker-compose.yml`: O ficheiro de orquestração que define e interliga todos os serviços, permitindo que sejam executados em conjunto com um único comando.
...
2.  **Construa e inicie os contentores Docker:**
    Execute o seguinte comando na raiz do projeto. O `--build` garante que as imagens são construídas de acordo com as versões mais recentes dos Dockerfiles.
    ```sh
...
Para parar todos os serviços, pressione `Ctrl + C` no terminal onde o `docker-compose` está a ser executado, e depois execute `docker-compose down` para remover os contentores.
