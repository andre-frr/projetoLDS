...
async function update(call, callback) {
    ...
    try {
        ...
        let paramIndex = 1;
        const setClauses = keys.map(key => `${key} = $${paramIndex++}`).join(', ');
        const values = Object.values(dataObj);

        const {where, params: pkParams} = buildPKWhere(tableName, id);

        // Adjust WHERE clause parameter indices
        const adjustedWhere = where.replace(/\$(\d+)/g, (_, num) => `$${paramIndex++}`);
...
async function fetchRelation(tableName, entity, relation) {
    const relationMap = {
        ...
        'docente': {
            'areaCientifica': {table: 'area_cientifica', pk: 'id_area', fk: 'id_area'},
