...
(
    ...
    NOT
    NULL
    UNIQUE -- e.g., 'Licenciatura', 'Mestrado', 'Doutoramento', 'Agregação'
);
...
(
    ...
    CONSTRAINT uq_area_sigla UNIQUE
(
    sigla
)
    );
...
(
    ...
    CONSTRAINT uq_docente_email UNIQUE
(
    email
)
    );
...
(
    ...
    ON UPDATE CASCADE
    ON DELETE RESTRICT,
    id_area INTEGER NOT NULL REFERENCES area_cientifica
(
    id_area
)
    ON UPDATE CASCADE
    ON DELETE RESTRICT,
    ano_curso SMALLINT NOT NULL CHECK
(
    ano_curso
    BETWEEN
    1
    ...
    sem_curso SMALLINT NOT NULL CHECK
(
    sem_curso
    IN
(
...
(
    ...
    ON UPDATE CASCADE
    ON DELETE CASCADE,
    id_grau SMALLINT REFERENCES grau
(
    id_grau
)
    ON UPDATE CASCADE
