...
async function checkCoordenadorPermission(userId, action, resource, context) {
    ...
    const handlers = {
        ...
        [RESOURCES.DSD]: () => handleCoordenadorDsdPermission(userId, action, context),
 ...
 */
async function handleCoordenadorDsdPermission(userId, action, context) {
    // Coordenadores can only READ DSDs, cannot create/update/delete
    if (action === ACTIONS.READ) {
        return true; // Can read all DSDs (filtered by their courses in the endpoint)
    }

    // All other actions (CREATE, UPDATE, DELETE) are forbidden
    return false;
}
